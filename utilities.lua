...
function is_bot_owner(msg, real_owner)
    -- if real owner is true, the function will return true only if msg.from.id == config.admin.owner
    local id
...
local function extRem(hash, id, key)
    ...
    if not hash_exists then
        ...
        if next(ids) then
            ...
            if not id then
                -- if id and key are not provided, delete the whole hash
                local subhash_removed = 0
                for i, id in pairs(ids) do
                    -- delete each sub-hash, before delete the ids set
                    db:del(hash .. ':' .. id)
                    -- remove directly the entire sub-hash
                ...
                -- else, if id is provided then
                local id_exists = db:sismember(hash, id)
                if not id_exists then
                    return false, 'id does not exists'
                else
                    if key then
                        -- if the key is provided, then delete only the key
                        local res_key = db:hdel(hash .. ':' .. id, key)
                        if res_key == 0 then
                            -- if the key does not exists then
                            return false, 'key does not exists'
                        else
                            return true
                        end
                    else
                        -- if the key is not provided, then delete only id provided from the ids set, and the associated sub-hash
                        db:del(hash .. ':' .. id)
                        -- delete the whole sub-hash
                        db:srem(hash, id)
                        -- remove the id from the set of ids
                        return true, 1
                    end
                end
            end
