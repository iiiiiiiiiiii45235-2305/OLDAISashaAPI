...
pre_process = function(msg, ln)
    ...
    if not is_ignored(msg.chat.id, msg_type) then
        ...
        if msgs > max_msgs then
            ...
            if status == 'no' and not msg.cb then
                ...
                if res then
                    cross.saveBan(msg.from.id, 'flood')
                    -- save ban
                    if action == 'ban' then
                    ...
                    if msgs ==(max_msgs + 1) or msgs == max_msgs + 5 then
                        -- send the message only if it's the message after the first message flood. Repeat after 5
            ...
            return msg, true
            -- if an user is spamming, don't go through plugins
        end
    end
    ...
    if msg.media and not(msg.chat.type == 'private') and not msg.cb then
        ...
        if not(status == 'allowed') then
            ...
            if max_reached_var then
                -- max num reached. Kick/ban the user
                -- try to kick/ban
                if status == 'kick' then
                    res = api.kickUser(msg.chat.id, msg.from.id, ln)
                ...
                if res then
                    -- kick worked
                    cross.saveBan(msg.from.id, 'media')
                    -- save ban
    ...
    if msg.text and msg.text:find('([\216-\219][\128-\191])') then
        ...
        if arab_status == 'kick' or arab_status == 'ban' then
            ...
            if res then
                cross.saveBan(msg.from.id, 'arab')
                -- save ban
    ...
    if is_blocked(msg.from.id) then
        ...
        return msg, true
        -- if an user is blocked, don't go through plugins
    end
