...
local function action(msg, blocks, ln)
    ...
    if not is_mod(msg) then
        if msg.cb then
            -- show a pop up if a normal user tap on an inline button
    ...
    if blocks[1] == 'removewarn' and msg.cb then
        local user_id = blocks[2]
        print(msg.chat.id, user_id)
        local num = db:hincrby('chat:' .. msg.chat.id .. ':warns', user_id, -1)
        -- add one warn
        local nmax =(db:get('chat:' .. msg.chat.id .. ':max')) or 3
        -- get the max num of warnings
    ...
    if blocks[1] == 'warn' then
        ...
        -- add one warn
        local nmax =(db:get(hash_max)) or 3
        -- get the max num of warnings
        ...
        if tonumber(num) >= tonumber(nmax) then
            ...
            if not res then
                ...
                cross.saveBan(msg.reply.from.id, 'warn')
                -- add ban
                if type == 'ban' then
                    -- add to the banlist
                ...
                -- if kick/ban works, remove the warns
            ...
            api.sendKeyboard(msg.chat.id, text, keyboard, true, msg.message_id)
