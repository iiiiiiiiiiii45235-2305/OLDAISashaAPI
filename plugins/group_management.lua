...
local function run(msg, matches)
    if msg.cb then
        ...
        elseif matches[2] == 'BACKPERMISSIONS' then
            ...
            editMessageReplyMarkup(msg.chat.id, msg.message_id, keyboard_permissions_list(matches[4], matches[3], nil, matches[5] or false))
        elseif matches[2] == 'GRANT' then
            ...
            editMessageReplyMarkup(msg.chat.id, msg.message_id, keyboard_permissions_list(matches[5], matches[3], permissionsTable[tostring(matches[5])][tostring(matches[3])], matches[6] or false))
        ...
        elseif matches[2] == 'GOTOLOCKS' then
            ...
            editMessageReplyMarkup(msg.chat.id, msg.message_id, keyboard_settings_list(matches[3], 1, nil, matches[4] or false))
        elseif matches[2] == 'GOTOMUTES' then
            ...
            editMessageReplyMarkup(msg.chat.id, msg.message_id, keyboard_settings_list(matches[3], 2, nil, matches[4] or false))
        ...
        elseif matches[2] == 'time_ban' or matches[2] == 'time_restrict' then
            ...
            elseif matches[4] == 'SECONDS' or matches[4] == 'MINUTES' or matches[4] == 'HOURS' or matches[4] == 'DAYS' or matches[4] == 'WEEKS' then
                ...
                editMessageReplyMarkup(msg.chat.id, msg.message_id, keyboard_time_punishments(matches[2], matches[6], time, matches[7] or false))
        ...
        elseif matches[2]:match('^%d$') then
            ...
            if groupDataDictionary[matches[2]:lower()] then
                ...
                if updatePunishment then
                    ...
                    editMessageReplyMarkup(msg.chat.id, msg.message_id, keyboard_settings_list(matches[4], matches[3], groupDataDictionary[matches[2]:lower()], matches[5] or false))
                    mystat(matches[1] .. matches[2] .. matches[3] .. matches[4] ..(matches[5] or ''))
                else
                    -- info of variable
        ...
        if msg.from.is_owner or msg.chat.type == 'private' then
            ...
            if sendKeyboard(msg.from.id, logPages(msg.chat.id), keyboard_log_pages(msg.chat.id)) then
                ...
                if msg.chat.type ~= 'private' then
                    local message_id = getMessageId(sendReply(msg, langs[msg.lang].sendLogPvt, 'html'))
    ...
    if (msg.chat.type == 'group' or msg.chat.type == 'supergroup') and data[tostring(msg.chat.id)] then
        ...
        if matches[1]:lower() == 'rules' then
            ...
            if msg.from.is_mod then
                ...
                if not data[tostring(msg.chat.id)].rules then
                    oldResponses.lastRules[tostring(msg.chat.id)] = getMessageId(sendReply(msg, langs[msg.lang].noRules))
                    io.popen('lua timework.lua "deletemessage" "60" "' .. msg.chat.id .. '" "' ..(oldResponses.lastRules[tostring(msg.chat.id)] or '') .. '"')
                else
                    oldResponses.lastRules[tostring(msg.chat.id)] = getMessageId(sendReply(msg, langs[msg.lang].rules:gsub('X', msg.chat.title) .. '\n' .. data[tostring(msg.chat.id)].rules))
                end
                if tmp then
            ...
            if msg.from.is_mod then
                if msg.reply then
                    if msg.reply_to_message.date > os.time() - dateToUnix(0, 0, 48, 0, 0) then
                        ...
                        local message_id = getMessageId(sendReply(msg, langs[msg.lang].ok))
                        io.popen('lua timework.lua "deletemessage" "60" "' .. msg.chat.id .. '" "' .. msg.message_id .. ',' ..(message_id or '') .. '"')
                    else
                    ...
                    if delAll[tostring(msg.chat.id)].from and delAll[tostring(msg.chat.id)].to then
                        if delAll[tostring(msg.chat.id)].to > delAll[tostring(msg.chat.id)].from then
                            if delAll[tostring(msg.chat.id)].to - delAll[tostring(msg.chat.id)].from <= 200 and msg.from.is_owner then
                                ...
                                local message_id = getMessageId(sendReply(msg, langs[msg.lang].deletingMessages))
                                io.popen('lua timework.lua "deletemessage" "60" "' .. msg.chat.id .. '" "' .. msg.message_id .. ',' ..(message_id or '') .. '"')
        ...
        if matches[1]:lower() == 'muteslist' then
            ...
            if msg.from.is_mod then
                ...
                if sendKeyboard(msg.from.id, langs[msg.lang].mutesOf .. '(#chat' .. tostring(msg.chat.id):gsub("-", "") .. ') ' .. chat_name .. '\n' .. langs[msg.lang].settingsIntro, keyboard_settings_list(msg.chat.id, 2)) then
                    if msg.chat.type ~= 'private' then
                        local message_id = getMessageId(sendReply(msg, langs[msg.lang].sendMutesPvt, 'html'))
                        io.popen('lua timework.lua "deletemessage" "60" "' .. msg.chat.id .. '" "' .. msg.message_id .. ',' ..(message_id or '') .. '"')
                        return
                    end
                else
        ...
        if matches[1]:lower() == 'textualmuteslist' then
            ...
            if msg.from.is_mod then
                ...
                oldResponses.lastSettings[tostring(msg.chat.id)] = getMessageId(sendReply(msg, showSettings(msg.chat.id, msg.lang)))
                if tmp then
                    deleteMessage(msg.chat.id, tmp, true)
                end
                io.popen('lua timework.lua "deletemessage" "60" "' .. msg.chat.id .. '" "' .. msg.message_id .. '"')
        ...
        if matches[1]:lower() == 'settings' then
            ...
            if msg.from.is_mod then
                ...
                if sendKeyboard(msg.from.id, langs[msg.lang].settingsOf .. '(#chat' .. tostring(msg.chat.id):gsub("-", "") .. ') ' .. chat_name .. '\n' .. langs[msg.lang].settingsIntro, keyboard_settings_list(msg.chat.id, 1)) then
                    if msg.chat.type ~= 'private' then
                        local message_id = getMessageId(sendReply(msg, langs[msg.lang].sendSettingsPvt, 'html'))
                        io.popen('lua timework.lua "deletemessage" "60" "' .. msg.chat.id .. '" "' .. msg.message_id .. ',' ..(message_id or '') .. '"')
                        return
                    end
                else
        ...
        if matches[1]:lower() == 'modlist' then
            ...
            if msg.from.is_mod then
                ...
                oldResponses.lastModlist[tostring(msg.chat.id)] = getMessageId(sendReply(msg, modList(msg.chat.id, msg.lang)))
                if tmp then
                    deleteMessage(msg.chat.id, tmp, true)
                end
                io.popen('lua timework.lua "deletemessage" "60" "' .. msg.chat.id .. '" "' .. msg.message_id .. '"')
