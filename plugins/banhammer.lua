    ...
    if next(all) then
        for unban_time, info in pairs(all) do
            if os.time() > tonumber(unban_time) then
                ...
                -- hash needed to check if an user is already tempbanned or not
            end
        end
    end
end
...
local action = function(msg, blocks, ln)
    if msg.chat.type ~= 'private' then
        if is_mod(msg) then
            ...
            if msg.cb then
                -- if the user tap on 'unban', show the pop-up
