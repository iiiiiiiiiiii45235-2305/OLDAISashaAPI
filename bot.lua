...
bot_init = function(on_reload)
    -- The function run when the bot is started or reloaded.
    ...
    -- Load miscellaneous and cross-plugin functions.
...
on_msg_receive = function(msg)
    -- The fn run whenever a message is received.
    -- vardump(msg)
    if not msg then
    ...
    collect_stats(msg)
    -- resolve_username support, chat stats
    ...
    for i, plugin in pairs(plugins) do
        ...
        if stop_loop then
            -- check if on_each_msg said to stop the triggers loop
            break
        elseif plugin.triggers then
            if (config.testing_mode and plugin.test) or not plugin.test then
                -- run test plugins only if test mode it's on
