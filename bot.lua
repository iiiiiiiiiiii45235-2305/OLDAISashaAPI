    ...
    for name, plugin in pairs(plugins) do
        ...
        for k, pattern in pairs(plugin.patterns) do
            ...
            if matches then
                ...
                if pattern ~= "([\216-\219][\128-\191])" and pattern ~= "!!tgservice (.*)" and string.match(pattern, "^###.*BACK") == nil and string.match(pattern, "^###.*PAGE") == nil and string.match(pattern, "^###.*DELETE") == nil and not msg.media then
                    if msg.chat.type == 'private' then
                        if disabled then
