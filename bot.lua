    ...
    for name, plugin in pairs(plugins) do
        ...
        for k, pattern in pairs(plugin.patterns) do
            ...
            if matches then
                ...
                if pattern ~= "([\216-\219][\128-\191])" and pattern ~= "!!tgservice (.*)" and not (string.match(pattern, "^###(.*)BACK") or string.match(pattern, "^###(.*)PAGE") or string.match(pattern, "^###(.*)DELETE")) and not msg.media then
                    if msg.chat.type == 'private' then
                        if disabled then
